const SMALL_GAP = 8;
const BIG_GAP = 24;
var dataTweaksOptions = {
  hasSeparateListCategory: true,
...
const variableCategoryCallback = (workspace) => {
  ...
  if (!false && dataTweaksOptions.moveReportersDown) {
    result = moveReportersDown(result);
  }

  if (!false && dataTweaksOptions.separateLocalVariables) {
    result = separateLocalVariables(workspace, result);
  }

  if (false || !hasSeparateListCategory) {
    return result;
  }
...
vm.runtime.getBlocksXML = function (target) {
  ...
  hasSeparateListCategory = dataTweaksOptions.separateListCategory;
...
function updateDataTweaks() {
  ...
  if (dataTweaksOptions.separateListCategory !== hasSeparateListCategory) {
    if (vm.editingTarget) {
      vm.emitWorkspaceUpdate();
    }
  } else {
...
const Data_dynamicEnableOrDisable = () => {
  ...
  if (
    dataTweaksOptions.separateLocalVariables ||
    dataTweaksOptions.moveReportersDown
  ) {
    const workspace = Blockly.getMainWorkspace();
    if (workspace) {
      workspace.refreshToolboxSelection_();
